...
use pinocchio_log::log;  // Correct: use log! from pinocchio-log
...
pub struct NormalConfig {
    pub state: u8,      // 1 byte
    pub value: u64,     // 8 bytes - requires 8-byte alignment
...
pub struct PackedConfig {
    pub state: u8,      // 1 byte
    pub value: u64,     // 8 bytes, but now unaligned (starts right after u8)
    pub authority: Pubkey,  // 32 bytes
}
...
fn process_instruction(
    ...
    // What can go wrong overall: Program panic (crash), wrong values read (incorrect logic, e.g. fake authority allowing fund drain), or undefined behavior (UB) like memory corruption leading to weird bugs or exploits.

    let data = account.data;  // Correct: borrow_data() returns &[u8]

    // Pattern 1: Using transmute() with Unaligned Data
    // Why wrong: transmute() assumes the bytes are perfectly aligned for u64 (must start at memory address divisible by 8). If not, UB happens - program might crash or read garbage.
    ...
    // Why wrong: Packed structs have no padding, so multi-byte fields like u64 can start at unaligned addresses (e.g. after a u8). Casting creates references to unaligned data → UB.
    ...
    let value2 = packed.value;  // UB if unaligned
    log!("Pattern 2 value: {}", value2);

    // Pattern 3: Direct Field Access on Packed Structs
    // Why wrong: Accessing fields like .value creates a reference to unaligned data → UB, even if the struct fits in memory.
    // What can go wrong: Temporary unaligned reference causes crash or corruption → program fails to update balances correctly, locking funds or allowing double-spend.
    let packed = unsafe { &*(data.as_ptr() as *const PackedConfig) };
    let b_ref = &packed.value;  // UB: reference to unaligned u64
    let b_value = packed.value; // UB: temporary reference
    log!("Pattern 3 b_value: {}", b_value);

    // Pattern 4: Assuming Alignment Without Verification
    // Why wrong: Casting pointer without checking if it's aligned for the struct (e.g. u64 needs 8-byte alignment) → UB if misaligned.
    ...
    // Why wrong: read_unaligned() is for unaligned data, but the struct must have predictable layout (#[repr(C)]). With default #[repr(Rust)], layout is undefined → UB.
